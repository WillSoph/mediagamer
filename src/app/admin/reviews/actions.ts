...
async function recalculateGameScoreIfApproved(gameId: string, status: string) {
  ...
  if (scoreError) {
    throw new Error(
      `Operação concluída, mas houve erro ao recalcular score do jogo: ${scoreError.message}`
    );
  }
}
...
export async function createReview(formData: FormData) {
  ...
  await recalculateGameScoreIfApproved(gameId, status || "pending_review");
...
export async function updateReview(formData: FormData) {
  ...
  if (!reviewId) {
    throw new Error("O ID da review é obrigatório para edição.");
  }
